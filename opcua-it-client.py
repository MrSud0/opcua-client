...
async def main(opc_ua_server_url, username, password):
    client = Client(opc_ua_server_url)

    auth_type = input("Enter authentication type (anonymous/userpass): ").strip().lower()
    ...
    try:
        ...
        while True:
            ...
            if action == "read":
                node_id = input("Enter the node ID (e.g., ns=2;i=2003): ").strip()
                node = client.get_node(node_id)
                value = await node.read_value()
                log.info(f"Value of node {node_id}: {value}")
            elif action == "write":
                node_id = input("Enter the node ID (e.g., ns=2;i=2003): ").strip()
                value = input("Enter the value to write: ").strip()
                node = client.get_node(node_id)
                try:
                ...
                log.error("Invalid action. Please enter 'read', 'write', or 'exit'.")
...
if __name__ == "__main__":
    ...
    asyncio.run(main(opc_ua_server_url, args.username, args.password))
